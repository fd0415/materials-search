...
供应商：**DeepSeek**（OpenAI 兼容接口）。密钥与模型 ID 从环境变量读取，集中在 `lib/ai/models.ts`，**不写死**：
```ts
export const AI = {
  baseURL: process.env.DEEPSEEK_BASE_URL ?? 'https://api.deepseek.com',
  apiKey:  process.env.DEEPSEEK_API_KEY!,
  models: {
    brain:      process.env.DEEPSEEK_MODEL_PRO!,    // 高推理：Agent 大脑（规划/判断/工具调用）
    copywriter: process.env.DEEPSEEK_MODEL_PRO!,    // 高推理：写配文
    judge:      process.env.DEEPSEEK_MODEL_FLASH!,  // 低推理：打分挑选
    tagger:     process.env.DEEPSEEK_MODEL_FLASH!,  // 低推理：离线打标签
  },
};
```
分配规则：
- **高推理 → DeepSeek V4 Pro**：大脑（Agent 编排循环）、写配文。大脑需 function-calling 驱动工具循环。
- **低推理 → DeepSeek V4 Flash**：打分挑选、离线打标签（量大，便宜快）。
- 大脑推理循环**从头到尾用同一个 Pro 模型**，保持上下文连贯，中途别换。
- 供应商可插拔：模型 ID 全走环境变量，将来换 DeepSeek 其他版本或换 Claude/GPT，只改 env 不动业务代码。
